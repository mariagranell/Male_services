...
overdisp.test<-function(x){
	...
	##additions/changes: properly treats models fitted with glmmTMB when correlation parameters were excluded in random effects structure
  ...
  if(class(x)[[1]]=="glmerMod"){
    ...
    n.model.terms=n.model.terms+length(summary(x)$theta)
		...
		n.model.terms=n.model.terms+sum(unlist(lapply(summary(x)$varcor, function(y){
			if(!is.null(y)){
				lapply(y, function(yy){
					...
					return(length(attr(yy, "stddev")) + ifelse(any(as.matrix(yy)[upper.tri(as.matrix(yy))]!=0), (xx[1]^2-xx[1])/2, 0))
				})
			}else{
				0
			}
		...
		sum.dp= sum((as.vector(pr)/sqrt(fitted.var))^2)
...
#     Season + zCSI + asr + TenureLeft + Season:mount_coming12 +
#     Unhabituated + Group",
...
# then you have a look at the supprt table. If tru you can consider adding a random slope for that variable.
...
fe.re.tab<-function(fe.model, re, other.vars=NULL, data, treat.covs.as.factors=c(NA, F, T)){
	...
	if(nrow(to.do)>0){
		...
		to.add=lapply(1:nrow(to.do), function(xrow){#treat combinations of fixed and random effect by lapply-ing over the rows of to.do
			...
			if(all(imodes=="factor")){
				...
				comb.fac=apply(comb.fac, 1, paste, collapse="_@_")
				ires=table(data[,to.do$re[xrow]], comb.fac)
				cn.mat=matrix(unlist(strsplit(colnames(ires), split="_@_", fixed=T)), ncol=2, byrow=T)
				ires.summary=t(sapply(1:nrow(ires), function(r){
          i.rr=cn.mat[which(ires[r, ]>1), , drop=F]
          apply(i.rr, 2, function(ic){length(unique(ic))})
        }))
        colnames(ires.summary)=unlist(strsplit(to.do[xrow, "int"], split=":", fixed=T))
        ires.summary=aggregate(1:nrow(ires.summary), as.data.frame(ires.summary), length)
        colnames(ires.summary)[ncol(ires.summary)]="freq"
        #ires.summary[, -ncol(ires.summary)][ires.summary[, -ncol(ires.summary)]>1]=">1"
        #ires.summary[, -ncol(ires.summary)][ires.summary[, -ncol(ires.summary)]==1]="!>1"
        #ires.summary[, -ncol(ires.summary)][ires.summary[, -ncol(ires.summary)]=="1"]="!>1"
				#class(ires)="matrix"
	...
	if(length(to.code)>0){
		...
		xnames=unlist(lapply(to.code, function(xc){#determine column names to be given to the matrix...
			paste(xc, levels(data[, xc])[-1], sep="../male_services_index/lukas_scripts")
		}))
...
flag_support <- function(x) {
  # unwrap if this is an interaction entry (list with $summary)
  if (is.list(x) && !is.null(x$summary)) x <- x$summary

  # if it's a data.frame (often for factor interactions), reduce to a single "supported?" flag
  if (is.data.frame(x)) {
    if (!("freq" %in% names(x))) return(NA)
    return(any(x$freq > 0, na.rm = TRUE))
  }

  # if it's a table / named numeric vector
  if (is.table(x) || (is.atomic(x) && !is.null(names(x)))) {
    v <- as.numeric(x)
    nms <- names(x)

    pattern_num <- suppressWarnings(as.integer(str_extract(nms, "\\d+")))
    return(any(pattern_num >= 2 & v > 0, na.rm = TRUE))
  }
