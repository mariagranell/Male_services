...
####  Create an observation number column --------------
# use like this: d <- obs.nr(data, data$Date, data$Time, data$Group)
obs.nr <- function(df, colDate= "Date", colTime = "Time", colGroup ="Group"){

# create a Obs.nr column
df$Obs.nr <- NA

obs_nr <- 1 # counter
  for(row.nr in seq_along(colDate)) {
    if(row.nr == 1) {
      df$Obs.nr[row.nr] <- 1
    } else {
      time1 <- as.numeric(as.POSIXct(colTime[row.nr], format = "%H:%M:%S")) +
               as.numeric(as.POSIXct(colDate[row.nr], format = "%Y-%m-%d"))
      time0 <- as.numeric(as.POSIXct(colTime[row.nr - 1], format = "%H:%M:%S")) +
               as.numeric(as.POSIXct(colDate[row.nr - 1], format = "%Y-%m-%d"))
      timeinterval <- time1 - time0
      if(colGroup[row.nr] != colGroup[row.nr - 1] |
        (timeinterval > 900) |
        colDate[row.nr] != colDate[row.nr - 1]) {
            obs_nr <- obs_nr + 1
      }
      df$Obs.nr[row.nr] <- obs_nr
    }
  }
  return(df)
}
...
# whole dataframe but just the desired column
 add_age <- function(data, birthdate = DOB , date = Sys.Date(), unit = NULL ){
  if (unit == "Months"){
    data %>%
...
#add_age <- function(data, birthdate, date = Sys.Date(), unit) {
#  if (unit == "Months") {
#    data %>%
#      mutate_at(vars(birthdate, date), as.Date) %>%
#      mutate(Age_Months = round(as.numeric(difftime({{date}}, {{birthdate}}, units = "days"))/30.4375, digits = 2))
#  } else if (unit == "Days") {
#    data %>%
#      mutate_at(vars(birthdate, date), as.Date) %>%
#      mutate(Age_Days = round(as.numeric(difftime({{date}}, {{birthdate}}, units = "days")), digits=2))
#  } else if (unit == "Years") {
#    data %>%
#      mutate_at(vars(birthdate, date), as.Date) %>%
#      mutate(Age_Years = round(as.numeric(difftime({{date}}, {{birthdate}}, units = "days"))/365.25, digits = 2))
#  } else {
...
# This function is used to analyze how many behaviours are courring in a behavioral string.

# This function takes two arguments: string_data and list_bhv.
# 1) string_data: is a character vector representing the strings that you want to search for specific behaviours.
# 2) list_bhv: is a character vector representing the behaviours that you want to search for in the string_data.
# The function returns an integer representing the number of times that any of the behaviours in list_bhv appear in string_data.
...
## to split behaviours into rows ------------------------

library(data.table)
library(splitstackshape)

split_behaviours <- function(data, cols_to_split, separator) {
  for (col in cols_to_split) {
    data <- suppressWarnings(cSplit(data, col, separator, "long"))
  }
  return(data)
}

# In this way each behaviour is going to be in a row. Instead of in sentences.s
# Usage:
# cols <- c("BehaviourIndividual1", "BehaviourIndividual2")
# data <- split_behaviours(data, cols, " ")

# when an old datafile has dates in different formats
# Example:
# b$Date <- as.Date(unlist(lapply(b$Date, tryFormats)), origin = "1970-01-01")
tryFormats <- function(date) {
  result <- as.Date(date, format = "%d/%m/%Y")
  if (is.na(result)) {
    result <- as.Date(date, format = "%m/%d/%Y")
  }
  return(result)
}
...
integrate_otherid <- function(df, ...) {

  # Capture multiple column names
  id_columns <- enquos(...)

  # Remove spaces from the specified columns in df
  df <- df %>%
    mutate(across(all_of(map_chr(id_columns, rlang::as_name)), ~ str_replace_all(.x, " ", "")))

  # Remove spaces from the OtherID column in KeyOtherID
  key_df <- KeyOtherID %>%
    mutate(OtherID = str_replace_all(OtherID, " ", ""))

  # Create named vector of replacements
  replacements <- setNames(key_df$AnimalCode, key_df$OtherID)

  # Apply replacements across all specified columns
  df <- df %>%
    mutate(across(all_of(map_chr(id_columns, rlang::as_name)),
                  ~ if_else(.x %in% names(replacements),
                           replacements[.x],
                           .x)))
...
# this is for when you have a dataframe with different time formats. This function does:
# Leaves proper hms/character-formatted times (e.g., "06:12:13") alone. Converts numeric or "hh.mm" strings to hh:mm:ss" format, setting seconds to 0.
#aa <- mutate(contacttime = vapply(as.character(contacttime), standardize_time, character(1)))
...
standardize_time <- function(x) {
  ...
  if (stringr::str_detect(x, "^\\d{1,2}:\\d{2}(:\\d{2})?$")) {
    # Add :00 seconds if missing
    if (!stringr::str_detect(x, ":\\d{2}$")) x <- paste0(x, ":00")
    return(x)
  }

  # Try converting numeric format like 11.4 or 6.28
  suppressWarnings({
    x_num <- as.numeric(x)
  })
  ...
  if (!is.na(x_num)) {
    hours <- floor(x_num)
    # Treat decimal part as minutes in base 100, not fraction of an hour
    minutes <- round((x_num - hours) * 100)
  ...
  return(NA_character_)
   ...
   # IF ALL FAILS you can create a datetime like this
#mutate(hours = floor(starttime),  # Extract hours
#    minutes = as.integer((starttime - floor(starttime)) *100),  # Extract minutes without rounding
#    approxstarttime = make_datetime(
#      year = year(Date),
#      month = month(Date),
#      day = day(Date),
#      hour = ifelse(is.na(hours),hours(Time), hours),
#      min = ifelse(is.na(minutes),minutes(Time), minutes),
#      sec = 0
#    )
